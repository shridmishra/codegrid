...
const PV = {
  config: {
    ...
    maxDisplacement: 500,
    returnForce: 0.05,
    logoPath: "/home/particle-visual.png",
    particleSpacing: 5,
  },
  canvas: null,
  gl: null,
...
function getParticleCanvasSize() {
  ...
  const container = canvas.closest(".particle-canvas") || canvas.parentElement;
  const rect =
...
function computeLogoScale(cssW) {
  const base = Math.min(cssW / 1920, 1);
  // Mobile needs a boost so the logo doesn't look tiny.
  // Clamp to avoid the earlier "too huge" behavior.
  if (cssW < 1000) return Math.min(base * 2.2, 0.9);
  return base;
}
...
function init() {
  ...
  // Only set backing resolution; keep CSS sizing controlled by `css/home.css`
  // (width:100%, height:100svh) so layout doesn't drift on mobile.
...
function handleResize() {
  // Use the actual element size (100svh/100% container) instead of
  // innerWidth/innerHeight to avoid "zoomed" scaling on mobile browsers.
  const { cssW, cssH } = getParticleCanvasSize();
